...
const swaggerDocument: OpenAPIV3.Document = {
  ...
  paths: {
    ...
    '/health': {
      get: operation('Health check', ['System']),
      head: operation('Health status check', ['System']),
    },
    '/sitemap.xml': { get: operation('Get the service sitemap', ['System']) },
    ...
    '/api/auth/register': { post: operation('Register a user', ['Auth']) },
    '/api/auth/registerDriver': { post: operation('Register a driver', ['Auth']) },
    '/api/auth/otp/register': { post: operation('Start OTP registration', ['Auth']) },
    '/api/auth/otp/verify': { post: operation('Verify registration OTP', ['Auth']) },
    '/api/auth/forgot-password': { post: operation('Request password recovery', ['Auth']) },
    '/api/auth/reset-password': { post: operation('Reset password', ['Auth']) },
    '/api/auth/password-reset/request': { post: operation('Request password reset link', ['Auth']) },
    '/api/auth/password-reset/validate': { post: operation('Validate password reset token', ['Auth']) },
    '/api/auth/password-reset/complete': { post: operation('Complete password reset', ['Auth']) },
    '/api/auth/login': { post: operation('Log in', ['Auth']) },
    '/api/auth/login/dash': { post: operation('Log in to dashboard', ['Auth']) },
    '/api/auth/refresh': { post: operation('Refresh access token', ['Auth']) },
    '/api/auth/logout': { post: operation('Log out', ['Auth']) },
    '/api/auth/me': { get: operation('Get current user', ['Auth'], true) },
    '/api/auth/google': { get: operation('Start Google OAuth', ['Auth']) },
    '/api/auth/google/callback': { get: operation('Handle Google OAuth callback', ['Auth']) },
    ...
    '/api/users/': {
      ...
      post: operation('Create a user', ['Users'], true),
    },
